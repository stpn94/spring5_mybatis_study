...
constraint fk_tutors_addr foreign key (addr_id) references mybatis_study.addresses (addr_id) 
) comment '교수';
...
constraint fk_course_tutor foreign key (tutor_id) references mybatis_study.tutors (tutor_id)
) comment '과목';
